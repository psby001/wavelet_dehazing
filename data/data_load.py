...
def train_dataloader(path, batch_size=64, num_workers=0, data='ITS', use_transform=True):
    ...
    transform = None
    if use_transform:
        pairrandomcrop = PairRandomCrop(crop_size)
        pairrandomcrop.pad_if_needed =True
...
def valid_dataloader(path, data, batch_size=1, num_workers=0):
    ...
    dataloader = DataLoader(
        DeblurDataset(os.path.join(path, 'test'), data, transform=transform),
        batch_size=batch_size,
        shuffle=False,
        num_workers=num_workers
    )
