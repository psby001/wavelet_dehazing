...
def main(args):
    ...
    ab_test_dir = creat_dir(os.path.join(args.OUTPUT_PATH, dataset_name))
    ...
    SsimLoss = CW_SSIM(imgSize=TRANSFROM_SCALES, channels=3, level=4, ori=8).to(device)
    SsimLoss_L = CW_SSIM(imgSize=(args.TRANSFROM_SCALES//2, args.TRANSFROM_SCALES//2), channels=3, level=4, ori=8).to(device)
    SsimLoss_D = CW_SSIM(imgSize=(args.TRANSFROM_SCALES//2, args.TRANSFROM_SCALES//2), channels=9, level=4, ori=8).to(device)
    ...
    for batch_idx, batch in tqdm.tqdm(enumerate(test_loader)):
        with torch.no_grad():
            ...
            # (output_map,ll_out,detail_out) = model(img)
            # (output_map,ll_out,detail_out) = model(img)
            ...
            loss = MSE(output_map, label)
            total_psnr += (10 * torch.log10(1 / F.mse_loss(output_map, label)).item())
            # total_ll_psnr += (10 * torch.log10(1 / F.mse_loss(ll, ll_label)).item())
            # total_detail_psnr += (10 * torch.log10(1 / F.mse_loss(detail, detail_label)).item())
            total_ssim += (SsimLoss.cw_ssim(output_map, label).item())
            total_ll_ssim += (SsimLoss_L.cw_ssim(ll, ll_label).item())
            total_detail_ssim += (SsimLoss_D.cw_ssim(detail, detail_label).item())
            ...
            for out_img,lls,details,hz_img,gt_img,name in zip(output_map,ll,detail,img,label,names):
    ...
    print("############################")
    print("loss_avg ", total_loss/len(test_loader),"SSMI ", total_ssim/len(test_loader) ,"PSNR ",total_psnr/len(test_loader))
    print("LL_SSMI ", total_ll_ssim / len(test_loader), "LL_PSNR ",
          total_ll_psnr / len(test_loader))
    print("Detail_SSMI ", total_detail_ssim / len(test_loader), "Detail_PSNR ",
...
def opt_args():
    ...
    args.add_argument('--weight_path', type=str, default="output/RESIDE-6K_UNet_wavelet_54/model_best.pth",
